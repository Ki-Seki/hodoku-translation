...
Rows and columns are numbered from 1 to 9 (left to right/top to bottom), a cell is specified by its row and column (e.g.: r5c2 means the cell at row 5 and column 2; r57c2 means cells r5c2 and r7c2). Blocks are numbered from 1 to 9 too (top most floor from left to right, then the next floor and so on).
...
*Content based on [HoDoKu](https://hodoku.sourceforge.net/en/tech_intro.php) by Bernhard Hobiger, licensed under GNU FDLv1.3.*
